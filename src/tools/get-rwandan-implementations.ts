...
export async function getRwandanImplementations(
  db: InstanceType<typeof Database>,
  input: GetRwandanImplementationsInput,
): Promise<ToolResponse<RwandanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as RwandanImplementationResult[];

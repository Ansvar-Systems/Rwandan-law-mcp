#!/usr/bin/env tsx
/**
 * Rwandan Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.minijust.gov.rw';
const PORTAL_NAME = 'Rwanda Ministry of Justice';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/rwandan-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Rwandan Law MCP — Data Freshness Check');
